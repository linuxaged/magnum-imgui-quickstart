...
class ArcBallCamera: public ArcBall {
    public:
            ...
            ArcBall{cameraPosition, viewCenter, upDir, fov, windowSize}
        {
            ...
            (*(_camera = new SceneGraph::Camera3D{*cameraObject}))
                ...
                .setProjectionMatrix(Matrix4::perspectiveProjection(
                    fov, Vector2{windowSize}.aspectRatio(), 1.0f, 800.0f))
        ...
        void zoom2(float value) {
            float x = _orthoViewportSize.x() - value;
            float y = _orthoViewportSize.y() - value;
            if(x > 96 || x < 5)
                return;
            _orthoViewportSize = {x, y};
            (*_camera)
                .setAspectRatioPolicy(SceneGraph::AspectRatioPolicy::Extend)
                .setProjectionMatrix(Matrix4::orthographicProjection(_orthoViewportSize, 1.0f, 800.0f));
        }

        void ortho(Vector2 windowSize) {
            (*_camera)
                .setAspectRatioPolicy(SceneGraph::AspectRatioPolicy::Extend)
                .setProjectionMatrix(Matrix4::orthographicProjection(_orthoViewportSize, 1.0f, 800.0f));
        }

        void project(Deg fov, Vector2 windowSize) {
            (*_camera)
                .setAspectRatioPolicy(SceneGraph::AspectRatioPolicy::Extend)
                .setProjectionMatrix(Matrix4::perspectiveProjection(
                    fov, windowSize.aspectRatio(), 1.0f, 800.0f));
        }
    ...
    private:
        ...
        Vector2 _orthoViewportSize{50.0f, 50.0f};
};
